...
import DaBridgeContracts from './da-bridge/DaBridgeContracts';
...
export class DeployContext extends MultiProvider<config.Domain> {
    ...
    pushVerification(
        ...
        const name = this.resolveDomainName(nameOrDomain);
        if (!this.verification.has(name)) {
            this.verification.set(name, []);
        }
    ...
    protected async deployBridge(name: string): Promise<void> {
        ...
        const bridge = new DaBridgeContracts(this, name);
    ...
    async ensureCoreConnections(): Promise<void> {
        ...
        await Promise.all(
            this.networks.map(async (network): Promise<void> => {
                ...
                // TODO there is only one network - eth
                ...
                const firstReplicaTxns = await core.enrollRemote("goerli");
                this.output();
