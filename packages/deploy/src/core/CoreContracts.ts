...
export default class EvmCoreDeploy extends AbstractCoreDeploy<config.EthereumCoreDeploymentInfo> {
    protected keys: ReadonlyArray<keyof config.EthereumCoreDeploymentInfo> = [
    ...
    constructor(
        ...
        domain: string,
        data?: config.EthereumCoreDeploymentInfo,
    ) {
        ...
        if (!utils.equalIds(owner, deployer)) {
            const call =
                await this.governanceRouter.populateTransaction.setRouterLocal(
                    2000,//remoteConfig.domain, TODO double check domain and read from config
